...
let user_details = {}
    ...
    function getDirectLoginToken() {
        if (localDirectLoginTokenIsValid()) {
            ...
            let direct_login_tokenValue = undefined
            createNewDirectLoginToken().then((value) => {
            console.log('returned value: ' + value)
            direct_login_tokenValue = value
        })
        return direct_login_tokenValue
        }
    }
    ...
    function localDirectLoginTokenIsValid() {
      if (directLoginTokenExistsIsLocallyWSE()) {
        ...
        cordova.plugin.http.post('https://apisandbox.openbankproject.com/obp/v4.0.0/users/current', Authorization: `DirectLogin token = "${window.localStorage.getItem( 'direct_login_token')}"`,}, {},
          function (response) {
    ...
    function directLoginTokenExistsIsLocallyWSE() {
      if (window.localStorage['direct_login_token']) {
        console.log(`token exists locally: ${window.localStorage.getItem('direct_login_token')}`)
        return true
      } else {
    ...
    function createNewUser() {
      ...
      return new Promise((resolve, reject) => {
        ...
        const createUserOptions = {
          method: 'post',
          data: {
            email: uuid_string + 1010 + '@example.com',
            username: uuid_string + 1010,
            password: uuid_string + 1010,
            first_name: uuid_string + 1010,
            last_name: uuid_string + 1010,
          },
        }
    ...
    function storeDirectLoginTokenWSE(direct_login_token) {
      var storage = window.localStorage
      storage.setItem('token', direct_login_token)
      // showPopUp("Token from storage: " + storage.getItem("token"))
      return direct_login_token
    }
    ...
    function getNewDirectLoginTokenWSE(username, password) {
      ...
      cordova.plugin.http.post(
        'https://test.openbankproject.com/my/logins/direct',
        {},
        {},
        function (response) {
          ...
          // showPopUp(res.token);
          return storeNewDirectLoginToken(res.direct_login_token)
        },
        function (response) {
    ...
    function openSofit() {
      ref = cordova.InAppBrowser.open(
        'https://test.openbankproject.com/user_mgt/login',
        '_blank',
        'location=no',
      )
