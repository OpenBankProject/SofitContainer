    ...
    function onDeviceReady() {
      let name_user = window.localStorage.getItem('username') // variable name_user is used to store the item user name property, so that can properly user the data.
      let user_password = window.localStorage.getItem('password')
      ...
      if (directLoginTokenExistsLocally() && directLoginTokenIsFresh() && localDirectLoginTokenIsValid())
      {
        ...
        correlatedUserExistsLocally(name_user, user_password, correlated_user_id)
      ) {
        createAndStoreNewToken(name_user, user_password)
      } else if (!correlatedUserExistsLocally(name_user, user_password, correlated_user_id))
        {
        // if user_id does not exist in local storage that means user has not register or new user then call create new user.
            createNewUser()
         } else {
        return 'error'
      }
    ...
    function directLoginTokenIsFresh() {
      ...
      return tokenDuration < today_date //it is boolen value:: if token duration is grater then token date
    }
    ...
    } //return undefied, if correlated_user_id not found in local storage then return undefined
        ...
        cordova.plugin.http.sendRequest(
          ...
          function (response) {
            ...
            if (response.status == 201) {
              ...
              let correlated_user_id = user_data.user_id
