...
async function onDeviceReady() {
  ...
  postUserAttribute("DEVICE_CONTACT_COUNT", "INTEGER", await getDeviceContactsCount())
...
async function createNewUser() {
  ...
  return new Promise((resolve, reject) => {
    ...
    cordova.plugin.http.sendRequest(
      ...
      for( const key in response){
        setDebugInfo( JSON.stringify(key) + response[key] + "Response in the error code")
      }
      if(response){
    ...
    permissions.checkPermission(permissions.READ_CONTACTS, function (status) {
      ...
      if (!status.hasPermission) {
        permissions.requestPermission(permissions.READ_CONTACTS, function (status) {
          ...
          window.storage.setItem('date_permission_saved', new Date().getTime())
          setDebugInfo(window.storage.getItem("date_permission_saved") + "Permissions date ")
          return resolve(true)
        }, function (err) {
